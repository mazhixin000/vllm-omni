...
async def edit_images(
    ...
    try:
        ...
        if len(stage_configs) > 1:
            ...
            images, stage_durations, peak_memory_mb, cot_output, postprocess_meta = generation_result
        else:
            ...
            images = _extract_images_from_result(result)
            # Mirror /v1/images/generations: surface stage timings and memory
            # peak so image edits are measurable with the same metric.
            stage_durations = getattr(result, "stage_durations", None)
            peak_memory_mb = getattr(result, "peak_memory_mb", None)
